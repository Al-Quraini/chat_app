...
class FirestoreService {
    ...
    try {
      await _firestore
        ...
        for(QueryDocumentSnapshot snapshot in snapshots.docs) {
          if (snapshot.id == AuthenticationService.getCurrentUserUid()!)
            continue;
  ...
  Future<bool> sendMessage({required u.User messageTo, required ChatMessage message}) async {
    try {
      ...
      await documentReference
          .set({
        'contacts': [AuthenticationService.getCurrentUserUid()!,messageTo.uid],
      }).then((value) {
  ...
  Future<void> addTexts(Map<String, String> map) async {
    // Call the user's CollectionReference to add a new user
    try {
      await _firestore
          .collection('texts')
  ...
  Future<bool> readMessage({required u.User user}) async{
    ...
    try {
      await _firestore
          ...
          AuthenticationService.getCurrentUserUid()!)).collection('messages')
          .where('sentBy', isNotEqualTo: AuthenticationService.getCurrentUserUid()!)
          .get().then((QuerySnapshot snapshots) async {
  ...
  Stream<QuerySnapshot> messagesStream({required String uid}){
    return  _firestore.collection('chats')
        .doc(
        generateChatUid(AuthenticationService.getCurrentUserUid()!, uid))
  ...
  List<ChatMessage> getMessages(AsyncSnapshot<QuerySnapshot> snapshots) {
    ...
    try {
      ...
      for (QueryDocumentSnapshot snapshot in snapshots.data!.docs) {
        ...
        chatMessage = ChatMessage(
          message: data['text'],
          type: data['sentBy'] == AuthenticationService.getCurrentUserUid()! ?
          MessageFrom.Sender :
    ...
    try {
      await _firestore.collection('chats').get()
        ...
        for (QueryDocumentSnapshot snapshot in snapshots.docs) {
          ...
          if(chatUser.senders.contains(AuthenticationService.currentUser))
